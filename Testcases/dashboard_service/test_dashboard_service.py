...
def test_dashboard_specific_client_success(client: TestClient, db_session):
    """
    Verify dashboard summary returns data for a specific client and department.
    """
    seed_dashboard_data(db_session)
...
def test_dashboard_start_month_and_year_error(client: TestClient):
    """
    Verify error is returned when both start_month and selected_year are provided.
    """
    payload = {
        "clients": "ALL",
        "start_month": "2024-01",
...
def test_dashboard_start_after_end_error(client: TestClient, db_session):
    """
    Verify that providing a start_month after end_month raises a 400 error
    with the correct detail message.
    """
    seed_dashboard_data(db_session)
    ...
    assert resp.status_code == 400
    assert resp.json()["detail"] == "start_month must be <= end_month"
