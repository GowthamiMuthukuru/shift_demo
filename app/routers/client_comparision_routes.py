...
from schemas.dashboardschema import ClientTotalAllowanceFilter,DashboardFilter,DashboardResponse,DeptDashboardFilter,DepartmentDashboardResponse,DepartmentTotalAllowanceFilter
from services.client_comparision_service import (client_comparison_service,
                                                 get_client_total_allowances,
                                                 get_client_departments_service,get_client_dashboard,get_department_dashboard,get_department_total_allowances)
...
@router.post("/department-total-allowances-piechart")
def department_total_allowances(
    filters: DepartmentTotalAllowanceFilter,
    top: Optional[str] = Query(None, description="Top N departments or 'ALL'"),
    db: Session = Depends(get_db),
    _current_user = Depends(get_current_user),
):
    # Inject top into filters
    ...
    return get_department_total_allowances(db, filters)